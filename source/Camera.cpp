...
namespace dae
{
	...
	void Camera::Update(const Timer* pTimer)
	{
		...
		const float mouseMovementSpeed{ 10.0f };
		...
		direction += (pKeyboardState[SDL_SCANCODE_W] || pKeyboardState[SDL_SCANCODE_UP]) * m_Forward * keyboardMovementSpeed * deltaTime;
		direction -= (pKeyboardState[SDL_SCANCODE_S] || pKeyboardState[SDL_SCANCODE_DOWN]) * m_Forward * keyboardMovementSpeed * deltaTime;
		direction -= (pKeyboardState[SDL_SCANCODE_A] || pKeyboardState[SDL_SCANCODE_LEFT]) * m_Right * keyboardMovementSpeed * deltaTime;
		direction += (pKeyboardState[SDL_SCANCODE_D] || pKeyboardState[SDL_SCANCODE_RIGHT]) * m_Right * keyboardMovementSpeed * deltaTime;
